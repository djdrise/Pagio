...
Windows names the program in “Open with” and in the file properties by the
executable's `FileDescription`, and electron-builder fills that field from the
`description` in `package.json` — a whole sentence there lands in the list.
`build.extraMetadata.description` overrides it with plain `Pagio` for the
packaged app only: the npm description stays as it is, and the Linux `.desktop`
comment comes from `build.linux.description` regardless.
