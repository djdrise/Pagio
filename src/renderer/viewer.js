...
function renderTabs() {
  ...
  for (const session of tabs) {
    ...
    // Полосы прокрутки у вкладок нет, утащить ленту мышью нечем:
    // активную подводим сами, иначе она останется за краем.
    if (current) session.tabEl.scrollIntoView({ block: 'nearest', inline: 'nearest' });
  }
}
