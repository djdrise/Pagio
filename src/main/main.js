...
function createWindow() {
  ...
  // Размер с прошлого раза мог достаться от монитора побольше. Просим не
  // больше рабочей области: окно, которое не влезло, Windows всё равно
  // подрежет, а страница внутри останется разложенной на прежнюю высоту.
  const { workArea } = useSaved ? screen.getDisplayMatching(saved) : screen.getPrimaryDisplay();

  win = new BrowserWindow({
    width: Math.min(saved?.width || 1200, workArea.width),
    height: Math.min(saved?.height || 820, workArea.height),
